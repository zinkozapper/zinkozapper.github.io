...
💻 Keylogger's Cold Sonnet: The TCP Exfiltration
...
The sacred pact of networking's trinity:sys/socket, arpa/inet—the channel to define,
...
And attempt the connect()—the promise it must keep.
...
An if (bytes_sent < key_len) check—the ultimate reprieve
Or final sealing of the victim’s fate.
...
int open_secure_channel() {
    ...
    // Attempt the connect()—the promise it must keep
...
void transmit_key(int sock) {
    ...
    // If (bytes_sent < key_len) check—the ultimate reprieve
    if (bytes_sent < key_len) {
